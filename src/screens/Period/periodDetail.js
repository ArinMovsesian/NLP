...
class PeriodDetail extends React.Component {
    ...
    componentDidMount() {
        ...
        AsyncStorage.getItem('token').then((value) => {
            ...
            axios.get('/Api/V1/period/' + this.props.itemID,
                {
                    headers: {
                        "uuid": "test-arin",
                        "token": value
                ...
                this.setState({
                    SpinnerIcon: false,
                    Images: response.data.data.periodInfo.images,
                    Address: response.data.data.periodInfo.address,
                    proID: response.data.data.periodInfo.id,
                    dataArray: [
                        {
                            title: response.data.data.periodInfo.title,
                            content: response.data.data.periodInfo.description
                        }
                    ],
                    PeriodUserTypeInfo: response.data.data.periodUserTypeInfo,
                });
            }).catch(error => {
        ...
        return (
            <Container>
                ...
                <Content padder>
                    ...
                    {
                        this.state.PeriodUserTypeInfo.map(
                            (value, index) => {
                                return (
                                    <Button full primary style={{borderRadius: 5, marginTop: 10}} key={value.id}>
                                        <View style={{ width: '100%', flexDirection: "row", padding: 10, justifyContent: "space-between"}}>
                                            <Text style={{color: '#FFF', fontFamily: 'iranyekanwebregular'}}>{value.price}</Text>
                    ...
                    {/*<View style={{ width: '100%', flexDirection: "row", padding: 10, justifyContent: "space-between"}}>*/}
                    {/*<Text style={{color: '#FFF', fontFamily: 'iranyekanwebregular'}}>{this.state.supperUserType}</Text>*/}
